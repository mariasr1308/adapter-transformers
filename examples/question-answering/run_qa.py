...
def main():
    ...
    def generate_adapter_config(switch):
        adapter_config = AdapterConfig.load(
                        adapter_args.adapter_config,
                        non_linearity=adapter_args.adapter_non_linearity,
                        reduction_factor=adapter_args.adapter_reduction_factor,
                        original_ln_before=switch[0], 
                        original_ln_after=switch[1], 
                        residual_before_ln=switch[2], 
                        adapter_residual_before_ln=switch[3], 
                        ln_before=switch[4], 
                        ln_after=switch[5], 
                        mh_adapter=switch[6],  
                        output_adapter=switch[7], 
                    )
    ...
    for switch in switches:
        ...
        if os.path.isdir(training_args.output_dir):
            print("#######################################################################")
            print("Skipping", training_args.output_dir)
            print("#######################################################################")
        else:
            print("#######################################################################")
            print("Processing", training_args.output_dir)
            print("#######################################################################")
        ...
        #         datasets = DatasetDict({"train": datasets[0], "validation": datasets[1]})
                ...
                datasets = DatasetDict({"train": datasets["train"], "validation": datasets_eval[1]})
            else:
                data_files = {}
    ...
    #                     )
        ...
        #             }
                def my_hp_space_optuna(trial):
                    return {
                        "learning_rate": trial.suggest_categorical("learning_rate",[1e-5,1e-3, 5e-3,1e-2]),
                        "seed":trial.suggest_categorical("seed", [25,38,42,87]),
